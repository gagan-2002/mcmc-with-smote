...
class MCMC:
    ...
    def sampler(self, w_limit, tau_limit):
        ...
        for i in range(samples - 1):
            ...
            #     X, y = oversample.fit_resample(X, y)
            #     y = y.reshape(X.shape[0],1)
            #     self.traindatadata = np.append(X,y,axis=1)
        ...
        auc1std = np.std(auc_afterburnin, dtype=np.float64)
        aucbest=np.amax(auc)
        print('AUC(MEAN): %.3f' % auc1)
        print('AUC(STD): %.3f' % auc1std)
        print('AUC(BEST): %.3f' % aucbest)
        pred_test=(pred_test>0.5)
        f111=np.mean(f11_afterburnin)
        f121=np.mean(f12_afterburnin)
        f131=np.mean(f13_afterburnin)
        f111std=np.std(f11_afterburnin, dtype=np.float64)
        f121std=np.std(f12_afterburnin, dtype=np.float64)
        f131std=np.std(f13_afterburnin, dtype=np.float64)
        print('f1 score weighted(MEAN):%.3f' % f111)
        print('f1 score macro(MEAN):%.3f' % f121)
        print('f1 score micro(MEAN):%.3f' % f131)
        print('f1 score weighted(STD):%.3f' % f111std)
        print('f1 score macro(STD):%.3f' % f121std)
        print('f1 score micro(STD):%.3f' % f131std)
...
def main():
    ...
    ecoli = pd.read_csv('winequality.data',sep=',')
    name	= "glass"
    ecolidat = pd.DataFrame(ecoli)
    # ecolidat = ecolidat.iloc[:,1:]
    ...
    traindata = np.delete(traindata,11,1)
    testdata = np.delete(testdata,11,1)
